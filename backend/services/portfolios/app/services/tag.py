...
from app.repositories import TagRepository, TaggableRepository
...
class TagService:
    ...
    async def get_entity_tags(self, entity_type: str, entity_id: int) -> list[Tag]:
        return await self.taggable_repo.get_tags(entity_type, entity_id)

    async def bulk_get_tags(self, items: list[tuple[str, int]]) -> dict[tuple[str, int], list[Tag]]:
        return await self.taggable_repo.bulk_get_tags(items)
