...
class RequestLogger:
    def __init__(self, provider_id: int, task_id: int, session_factory: async_sessionmaker[AsyncSession]) -> None:
    ...
    async def log(self, method: str, endpoint: str, response: httpx.Response | None,
        ...
        log = RequestLog(
            provider_id=self.provider_id, task_id=self.task_id, endpoint=endpoint,
